  ...
  module P2pOrderProcessing
    class AMQP < TradeExecution::AMQP
      ...
      def init_wait_orders_queue!
        orders = P2pOrder.where(state: 'prepare').shuffle
        @wait_orders_queue =
      ...
      def order_processing_queue_status
        @rmq_http_client.list_queues.find { |q| q[:name] == AMQP::Config.binding_queue(:p2p_order_processor).first }
      end
    end
  end
end
